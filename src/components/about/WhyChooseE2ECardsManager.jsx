...
import {
  ...
  Image as ImageIcon, X, Save, Loader2
} from 'lucide-react';
...
export default function WhyChooseE2ECardsManager() {
  ...
  const loadCards = async () => {
    ...
    try {
      ...
      const sorted = [...cardsData].sort((a, b) => (a.displayOrder ?? 0) - (b.displayOrder ?? 0));
      setCards(sorted);
    } catch (error) {
  ...
  const saveCard = async () => {
    ...
    try {
      if (editingId) {
        ...
        formDataPayload.append('displayOrder', Number(formData.displayOrder) || 0);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
      ...
      >
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-semibold text-gray-800">Why Choose E2E - Cards</h2>
          {isLoading ? (
