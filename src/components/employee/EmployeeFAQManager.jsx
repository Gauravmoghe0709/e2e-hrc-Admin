...
import {
  ...
  updateEmployeeFAQ,
  deleteEmployeeFAQ,
} from '../../services/employee/employeeFAQService';
...
export default function EmployeeFAQManager() {
  ...
  const fetchFAQs = async () => {
    try {
      setIsLoading(true);
      const res = await getAdminEmployeeFAQs();
  ...
  const saveFAQ = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateEmployeeFAQ(editingId, payload);
        toast.success('FAQ updated successfully!');
      } else {
        await createEmployeeFAQ(payload);
  ...
  const doDeleteFAQ = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteEmployeeFAQ(deletingId);
